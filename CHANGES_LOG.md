...
- **💾 按钮文案与行为对齐**：该按钮实际是把当前颜色存入「已保存」列表，文案由「保存为调色板」改回「保存颜色」/ "Save color"，避免与真正的「保存 25 色调色板」功能混淆
- **🌐 中英提示对齐**：中文关闭提示由「双击空白处关闭」统一为「双击关闭」，与英文 "Double-click to close" 一致（实际行为：双击面板任意非控件区域关闭）
- **🖱️ 吸附按钮提示刷新**：语言切换后悬停工具栏吸附按钮即可看到新语言的提示，无需刷新页面

### 清理

- **🧹 移除 `locales/` 目录**：该目录在仓库内无任何消费者（UI 文案唯一来源为 `main.js` 内置字典），且 Registry 安装包本就不包含它
